...
masterUrl = 'https://www.nrc.gov/reading-rm/doc-collections/enforcement/actions/'
baseURL = 'https://www.nrc.gov'
WebsiteDateTypes = ['reactors','non-licensees','materials','individuals','fuel-cycle-fac']
savedFile_directory = r'' # Your Path for save PDFs File  
...
log_path = "NCR.log"#Your Path for Log File  
...
def Extraction(): 
    for elements in WebsiteDateTypes:
        create_website_folder(masterUrl, elements)
        for letter in range(ord('a'), ord('z')+1):
            ATOZ= (chr(letter))
            yearURL = f'{elements}/{ATOZ}.html'
            ...
            for link in links:
                href = link.get("href")
                if href and href.endswith(".pdf") and href.startswith("https://www.nrc.gov"):
                    pdf_links = href.split('https://www.nrc.gov')[1]
                    download_pdfs(pdf_links)
                if href and href.endswith(".pdf") and not href.startswith("https://www.nrc.gov"):
                    pdf_links = href
                    download_pdfs(pdf_links)
...
if __name__ == '__main__':
    logger.info('Starting script- hometreasurygov.py')
    t1 = time.time()
    try:
