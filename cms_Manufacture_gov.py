...
masterUrl = 'https://www.cms.gov/Medicare/Compliance-and-Audits/Part-C-and-Part-D-Compliance-and-Audits/PharmaceuticalManufacturerEnforcementActions'
...
log_name = 'cms_LOG_manufracture.logs' # Log file name 
...
def create_website_folder(url):
    ...
    folder_ = domain_name.replace(".", "_") + 'Manufacturer'   # Replace any periods in the domain name with underscores
...
def extracting_pdfwebpage():
    for ittrate in range(pageno):
        paging = f'?items_per_page=100&combine=&page={ittrate}'
...
if __name__ == '__main__':
    logger.info('Starting script- CMS_gov_manufracture.py')
    t1 = time.time()
    try:
